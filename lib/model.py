...
def train_net(train_data_layer, net, epoch, args):
    ...
    for step in range(int(epoch_num)):
        ...
        loss = args.criterion((rel_so_prior+F.softmax(rel_score,dim=1)).view(1, -1), target)
        losses.update(loss.item(), n=1)
        loss.backward()
        args.optimizer.step()
        if step % args.print_freq == 0:
            time2 = time.time()
            print("EPOCH:%d, TRAIN:%d, Total LOSS:%f, Time:%s" % (epoch+1,step, losses.avg, time.strftime('%H:%M:%S', time.gmtime(int(time2 - time1)))))
            time1 = time.time()
            losses.reset()
...
def test_pre_net(net, args):
    ...
    print('CLS PRE TEST r50:%f, r50_zs:%f, r100:%f, r100_zs:%f'% (rec_50, rec_50_zs, rec_100, rec_100_zs))
    time2 = time.time()
